...
if profsave == "1" then
	if fs.stat("/usr/lib/country/mccdata") then
		...
		ddaa = m:section(TypedSection, "disable", translate("AutoAPN Mode"), translate("Switch between Standard and Database Automatic APN"))
		ddaa.anonymous = true
		aaenabled = ddaa:option(ListValue, "mode", translate("Mode"))
		aaenabled:value("1", translate("Database/Select by ISP"))
		aaenabled:value("0", translate("Standard"))
		...
		--dda.anonymous = true
		aenabled = ddaa:option(Flag, "mccapn", translate("Database Automatic APN Enabled"), translate("If Enabled will use the Database for APN. Otherwise will use the Default Profile."))
		aenabled.default="0"
		aenabled.optional=false;
		aenabled:depends("mode", "1")
		benabled = ddaa:option(Flag, "sort", translate("Sort ISP"), translate("This will sort the ISPs from the Database so the probable one is tried first"))
		benabled.default="0"
		benabled.optional=false;
		benabled:depends("mode", "1")
		
		dda = m:section(TypedSection, "roaming", translate("Roaming Settings"), translate("Settings that affect roaming"))
		dda.anonymous = true
		aenabled = dda:option(ListValue, "ipv4", translate("IPv4 Only when Roaming"))
		aenabled:value("1", translate("Yes"))
		aenabled:value("0", translate("No"))
		aenabled.default=0
		aenabled = dda:option(ListValue, "roam", translate("Enable Roaming"))
		aenabled:value("0", translate("Yes"))
		aenabled:value("1", translate("No"))
		aenabled.default=0
	end
